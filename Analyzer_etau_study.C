...
//
#define Analyzer_etau_study_cxx
#include "Analyzer_etau_study.h"
#include <TH2.h>
#include <TStyle.h>
#include <TCanvas.h>
#include "TH1F.h"
...
int main(int argc, const char* argv[])
{
  ...
  //std::string SampleName = argv[5];
  Analyzer_etau_study t(argv[1],argv[2]);
  t.Loop(maxEvents,reportEvery, SampleName);
  return 0;
}

void Analyzer_etau_study::Loop(Long64_t maxEvents, int reportEvery, string SampleName)
{
   ...
   Long64_t nentriesToCheck = nentries;
	...
	h_cutflow->SetBinContent(1,nInspected_genWeighted );
        ...
        h_cutflow->SetBinContent(4, nMuonDzPassed);
        h_cutflow->SetBinContent(5, nMuonD0Passed);
        h_cutflow->SetBinContent(6, nGoodTauPassed);
...
void Analyzer_etau_study::BookHistos(const char* file2)
{
...
void Analyzer_etau_study::fillHistos(int histoNumber, double event_weight, int higgs_Index)
{
...
std::vector<int> Analyzer_etau_study::getMuCand(double muPtCut, double muEtaCut){
...
std::vector<int> Analyzer_etau_study::found_higgs(){ 
...
std::vector<int> Analyzer_etau_study::getTauCand(double tauPtCut, double tauEtaCut){
  std::vector<int> tmpCand;
  tmpCand.clear();
  //Loop over taus      
...
std::vector<int> Analyzer_etau_study::getASRTauCand(double tauPtCut, double tauEtaCut){
  std::vector<int> tmpCand;
  tmpCand.clear();
  /*  //Loop over taus
  for(int iTau=0;iTau<nTau;iTau++)
...
bool Analyzer_etau_study::thirdLeptonVeto(){
...
double Analyzer_etau_study::dR(int mu_index, int tau_index)
{
...
double Analyzer_etau_study::delta_R(float phi1, float eta1, float phi2, float eta2)
{
...
double Analyzer_etau_study::DeltaPhi(double phi1, double phi2)
...
float Analyzer_etau_study::TMass_F(float LepPt, float LepPhi , float met, float metPhi) {
    return sqrt(pow(LepPt + met, 2) - pow(LepPt* cos(LepPhi) + met * cos(metPhi), 2) - pow(LepPt * sin(LepPhi) + met * sin(metPhi), 2));
}

float Analyzer_etau_study::TotTMass_F(TLorentzVector a, TLorentzVector b, TLorentzVector met) {
  float totalTMass = (a + b+ met).M();
  return totalTMass;
}


float Analyzer_etau_study::VisMass_F(TLorentzVector a, TLorentzVector b){
  float visibleMass = (a + b).M();
  return visibleMass;
}

float Analyzer_etau_study::pTvecsum_F(float pt1, float pt2, float phi1, float phi2) {
  float pt_vecSum = sqrt( pow(pt1*cos(phi1) + pt2*cos(phi2), 2) + pow(pt1*sin(phi1) + pt2*sin(phi2), 2));
  return pt_vecSum;
}

bool Analyzer_etau_study::passBjetVeto()
{
  int tmpCand = 0;
  bool veto = true;
...
bool Analyzer_etau_study::check_jets()
{
  int tmpCand=0;
...
double Analyzer_etau_study::prefiring_weight(TH2F* prefiring_photon, TH2F* prefiring_jet)
{ 
}
