...
void new_analyzer::Loop(Long64_t maxEvents, int reportEvery, string SampleName)
{
   ...
   for (Long64_t jentry=0; jentry<nentriesToCheck;jentry++)
       ...
       if(metFilters==0)
	 {
	   ...
	   if( HLTEleMuX>>21&1 == 1 ||
		 ...
		 {
		   if (leading_mPt < muPt->at(muCand[iMu]) )
		     {
		       leading_mPt = muPt->at(muCand[iMu]);
		       leading_muon = muCand[iMu];
		     }
		 }
	       ...
	       if( tauCand.size()>0 ) 
		 {
		   ...
		   for (int iTau=0; iTau<tauCand.size(); iTau++)
		     {
		       if (leading_tPt < tau_Pt->at(tauCand[iTau]) )
			 {
			   leading_tPt = tau_Pt->at(tauCand[iTau]);
			   leading_tau = tauCand[iTau];
			 }
		     }
		   
		   reco_mu.push_back( leading_muon );
		   reco_tau.push_back( leading_tau );
		   if ( (muCharge->at(leading_muon))*(tau_Charge->at(leading_tau))<0  ) 
		     {
...
std::vector<int> new_analyzer::getTauCand(double tauPtCut, double tauEtaCut, int mu_index, int &mu_selected){
  std::vector<int> tmpCand;
  tmpCand.clear();
  //Loop over taus      
